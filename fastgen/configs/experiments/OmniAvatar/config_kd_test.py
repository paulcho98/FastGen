...
"""Test config: ODE KD training on 3 Hallo3 samples, 20 iterations.

Requires ode_path.pt in each sample dir (run generate_omniavatar_ode_pairs.py first).

Usage:
    CUDA_VISIBLE_DEVICES=0 python train.py \
        --config fastgen/configs/experiments/OmniAvatar/config_kd_test.py
"""

import os
from omegaconf import DictConfig
from fastgen.configs.experiments.OmniAvatar.config_kd import create_config as create_base_config
...
def create_config():
    ...
    # Point to test data with ODE paths
    ...
    config.log_config.group = "omniavatar_kd_test"
    config.log_config.name = "kd_test_run"
