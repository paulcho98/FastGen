...
# teacher + GAN. Our OmniAvatar bs=8 default is tuned for no-GAN. bs=8, 4,
# and 2 all OOMed; bs=1 matches the reference recipe exactly.
#
# Effective batch preserved: 1 * 4 GPUs * 16 accum = 64.
#
# Usage:
#   bash scripts/train_sf_sink1_window7_redmd_beta2_audiofix_taew_gan_bs1.sh \
#     2>&1 | tee /tmp/train_sf_sink1_window7_redmd_beta2_audiofix_taew_gan_bs1.log
# =============================================================================
set -euo pipefail
...
export FASTGEN_OUTPUT_ROOT="/tmp/FASTGEN_SF_OUTPUT_BETA2_AUDIOFIX_TAEW_GAN_BS1"
...
RUN_NAME="sf_sink1_window7_redmd_audiofix_beta2_taew_gan_bs1"

echo "============================================="
echo "  Re-DMD beta=2 + TAEW + GAN @ batch=1 (matches reference)"
echo "============================================="
echo "  DF init ckpt:       ${OMNIAVATAR_DF_CKPT}"
echo "  gan_loss_weight:    0.003"
echo "  Discriminator:      Wan_14B"
echo "  Dataloader batch:   1 (matches WanT2V/config_sf_14b.py)"
echo "  Grad accum rounds:  16 (effective batch 1 * 4 GPUs * 16 accum = 64)"
...
/home/work/.local/miniconda3/envs/hb_fastgen/bin/torchrun \
    --nproc_per_node=4 \
    train.py \
    --config=fastgen/configs/experiments/OmniAvatar/config_sf_sink1_window7_redmd_beta2_taew_gan_bs1.py \
