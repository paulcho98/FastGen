...
def load_diffusion_model(args, device, dtype):
    ...
    #
    # 1.3B (legacy): merge_lora=True keeps the original behavior — the
    # OmniAvatar V2V LoRA is folded into base at construction, and the SF
    # checkpoint is expected to have plain (already-merged) keys.
    ...
    if args.model_size == "14B" and args.merge_lora_post_load:
        print(f"  Merging LoRA into base for inference speed...")
        ...
        if merge_count > 0:
            ...
            print(f"  WARN: no LoraLayer instances found; model has no PEFT "
                  f"adapters to merge (was --model_size set correctly?).")
