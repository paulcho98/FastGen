...
#
# Distinct output dir + wandb run name (_t769_lrcrit4e7 suffix) so it
# does NOT collide with fsmatched_lr3e6.
#
# Usage:
#   bash scripts/train_sf_sink1_window7_redmd_beta2_audiofix_taew_syncnet_mouthweight_fsmatched_t769_lrcrit4e7.sh
#
# Resume:
#   RESUME=True bash scripts/...t769_lrcrit4e7.sh
# =============================================================================

set -euo pipefail

# Distinct output dir and wandb run name.
export FASTGEN_OUTPUT_ROOT="${FASTGEN_OUTPUT_ROOT:-/tmp/FASTGEN_SF_OUTPUT_BETA2_AUDIOFIX_TAEW_SYNCNET_MOUTHWEIGHT_FSMATCHED_T769_LRCRIT4E7}"
export RUN_NAME="${RUN_NAME:-sf_sink1_window7_redmd_audiofix_beta2_taew_syncnet_mouthweight_fsmatched_t769_lrcrit4e7}"
...
export EXTRA_OVERRIDES="model.sample_t_cfg.t_list=[0.999,0.769,0.0] model.fake_score_optimizer.lr=4e-7"
