...
# =============================================================================
# =============================================================================
# Re-DMD Stage 2 — Audio-Fix Re-Run
# =============================================================================
# Same as train_sf_sink1_window7_redmd.sh, but run AFTER the
# concat(dim=0)->stack(dim=1) fix in
# fastgen/networks/OmniAvatar/{wan_model.py, network_causal.py}.
#
# IMPORTANT: point OMNIAVATAR_STUDENT_CKPT at the NEW DF checkpoint from the
# audio-fix DF re-run (train_omniavatar_df_audiofix.sh). The prior DF
# checkpoint was trained against scrambled audio (B=16 + concat bug) and
# should NOT be used as init — it has under-learned audio projections.
#
# Usage:
#   OMNIAVATAR_STUDENT_CKPT=/path/to/df_audiofix/step-XXXXX.pt \
#     nohup bash scripts/train_sf_sink1_window7_redmd_audiofix.sh \
#     > /tmp/train_sf_sink1_window7_redmd_audiofix.log 2>&1 &
# =============================================================================
set -euo pipefail
...
export FASTGEN_OUTPUT_ROOT="/tmp/FASTGEN_SF_OUTPUT_AUDIOFIX"
...
if [[ -z "${OMNIAVATAR_STUDENT_CKPT:-}" ]]; then
    ...
    echo "  Set it to the new DF checkpoint produced by" >&2
    echo "  train_omniavatar_df_audiofix.sh before launching." >&2
    exit 1
fi

RUN_NAME="sf_sink1_window7_redmd_audiofix_syncc_beta0p25"

echo "============================================="
echo "  Re-DMD Training (audio-fix re-run)"
echo "============================================="
