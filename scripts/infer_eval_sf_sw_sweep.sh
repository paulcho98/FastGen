#!/bin/bash
# Inference + evaluation sweep for ALL sliding-window SF checkpoints.
# Skips any checkpoint that already has completed inference output.
# Runs sequentially on a single GPU.
#
# Usage: nohup bash scripts/infer_eval_sf_sw_sweep.sh > /tmp/infer_eval_sf_sw_sweep.log 2>&1 &
set -uo pipefail
...
echo "============================================="
echo "  SF SW Sweep: ${#ALL_CKPTS[@]} checkpoints, GPU ${GPU_ID}"
echo "============================================="

for CKPT in "${ALL_CKPTS[@]}"; do
    STEP=$(basename "$CKPT" .pth)
    OUT_DIR="$OUT_ROOT/step_${STEP}"
    METRICS_OUT="$OUT_ROOT/metrics/step_${STEP}"
    ...
    echo "Inference done: ${num_generated}/${TOTAL_VIDEOS} videos"

    # --- Evaluation ---
    mkdir -p "$METRICS_OUT"
    pushd "$EVAL_METRICS_DIR" > /dev/null

    echo "[step ${STEP}] Eval Pass 1: SyncNet, LMD, CSIM"
    ...
    bash eval/run_metrics.sh \
        ...
        --ssim-lmd \
        --csim \
    || echo "[step ${STEP}] Pass 1 had failures (continuing)"

    echo "[step ${STEP}] Eval Pass 2: GT-aligned SSIM, FID, FVD"
    OMP_NUM_THREADS=4 MKL_NUM_THREADS=4 OPENBLAS_NUM_THREADS=4 NUMEXPR_NUM_THREADS=4 \
    CUDA_VISIBLE_DEVICES=$GPU_ID \
    bash eval/run_metrics.sh \
        --real_videos_dir "$GT_DIR" \
        ...
        --fake_videos_top_level \
        --gt-aligned \
    || echo "[step ${STEP}] Pass 2 had failures (continuing)"
