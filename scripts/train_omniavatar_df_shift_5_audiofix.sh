#!/bin/bash
# =============================================================================
# OmniAvatar DF (shift=5) — Audio-Fix Re-Run
# =============================================================================
#
# Same as train_omniavatar_df_shift_5.sh, but run AFTER the
# concat(dim=0)->stack(dim=1) fix in
# fastgen/networks/OmniAvatar/{wan_model.py, network_causal.py}. At per-rank
# batch >= 2 the old code scrambled (batch, projection) pairs for audio
# conditioning; this run retrains on correctly-routed audio.
#
# Starting checkpoint: same 1.3B V2V phase2 (step-19500) — pretrained against
# the canonical OmniAvatar reference (stack(dim=1)), so it is clean.
#
# Usage:
#   bash scripts/train_omniavatar_df_shift_5_audiofix.sh
#   NGPU=2 bash scripts/train_omniavatar_df_shift_5_audiofix.sh
#   MAX_ITER=10000 bash scripts/train_omniavatar_df_shift_5_audiofix.sh
# =============================================================================
...
RUN_NAME="df_audiofix_shift_5_${NGPU}gpu_bs${BATCH_SIZE}_lr1e5_${MAX_ITER}iter"

echo "============================================="
echo "  OmniAvatar DF shift=5 (audio-fix re-run)"
echo "============================================="
echo "  GPUs:            ${NGPU}"
